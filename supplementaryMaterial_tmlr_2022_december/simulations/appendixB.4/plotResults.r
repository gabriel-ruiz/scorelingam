...
fname <- 'simulations/appendixB.4/results.rdata'
#
load(fname)
dim(results)
#
...
p1a <- ggplot(dplyr::filter(res,famIndic==TRUE),aes(x=factor(ratio),y=error,fill=algorithm))+
  ...
  theme_classic()+
  ggtitle('Results without standardizing data matrix, 10 Most Correlated Nodes as Neighbors')+
  theme(
...
p1a
ggsave(filename = '~/UCLA/nonGauss_scms/simulations/26 November 2022/sortError_correctSpec_unscaled_10MostCorrelated.png',
...
p1b <- ggplot(dplyr::filter(res,famIndic==FALSE),aes(x=factor(ratio),y=error,fill=algorithm))+
  ...
  facet_grid(rows=vars(fam),cols=vars(network))+
  theme_classic()+
  ggtitle('Results without standardizing data matrix,\n10 Most Correlated Genes as Neighbors')+
  theme(
    plot.title = element_text( face='bold',size = 14,hjust=0.5),
    legend.title = element_text(face='bold', size = 12),legend.text=element_text( size = 12),
    axis.title = element_text(face = "bold", size = 14), axis.text = element_text( size = 13),
