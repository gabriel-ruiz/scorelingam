  ...
  for(nfac in rev(nfac2try)){
    ...
    for(f in 1:length(famSpecified)){
      for(r in 1:nreps){
        ...
        start <- Sys.time()
        perm.highd <- highDLingam::findGraphMulti(Y=X[inds.sort,],verbose = F,B = skel)$topOrder
        end <- Sys.time()
        stopCluster(cl)
        ...
        cat('..ScoreLingam..\n')
        start <- Sys.time()
        perm.score <- sort_llrmbCPP(Xmat=scale(X[inds.sort,]),mb=mbhat,numUpdates = 0,family=famSpecified[f],df=10)
        end <- Sys.time()
